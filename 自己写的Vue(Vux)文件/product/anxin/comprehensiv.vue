        ...
        <group :gutter='0'>
            ...
            :dialog-style="{'max-width': '90%', width: '90%' }">
                <div class="dialog-div">
                  <ol>
                    <li><p>1.本人自愿投保贵公司《安心综合医疗保险(2017 版)(A 款)》或《安心综合医疗保险(2017 版)(B 款)》，并声明以上陈述及各 项细节均真实无讹，且没有隐瞒任何重大事实以影响贵公司评估风险或接受本投保申请。  </p>
                    </li>
                    <li><p>2.本人已认真阅读《安心财产保险有限责任公司安心综合医疗保险(2017版)(A款) 条款》或《安心财产保险有限责任公司安心综合医疗保险(2017版)(B款) 条款》的所有条款，尤其是“不能获得赔偿的情形”并对贵公司就保险合同的内容说明和提 示完全理解，没有异议，申请投保。  </p>
                    </li>
            ...
            :dialog-style="{'max-width': '90%', width: '90%'}">
                <div class="dialog-div">
                  <div class="mdFormart_box_content_index">
                    ...
                    <p>（12）投保本保险前，请投保人认真阅读产品页面展示内容以及《安心综合医疗保险（2017版）（A款）》或《安心综合医疗保险（2017版）（B款）》条款。  </p>
            ...
            :dialog-style="{'max-width': '90%', width: '90%'}">
                <div class="dialog-div">
                  <a class="show-pdf" @click='showPDF(2, "安心综合医疗保险条款（2017版）")'>安心综合医疗保险条款（2017版）（A款）</a>
                  <br>
                  <br>
                  <a class="show-pdf" @click='showPDF(2, "安心综合医疗保险条款（2017版）")'>安心综合医疗保险条款（2017版）（B款）</a>
                </div>
                <section class="submit-button">
...
export default {
  ...
  created () {
    ...
    var comprehensivFlowData = this.storage.get('anxinComprehensivFlowData')
    if (comprehensivFlowData) {
      if (comprehensivFlowData.produceId) {
        this.flowData.produceId = comprehensivFlowData.produceId
      }
      if (comprehensivFlowData.vendorId) {
        this.flowData.vendorId = comprehensivFlowData.vendorId
      }
      if (comprehensivFlowData.socialInsuFlag) {
        this.flowData.socialInsuFlag = comprehensivFlowData.socialInsuFlag
      }
      if (comprehensivFlowData.policyHolderEmail) {
        this.flowData.policyHolderEmail = comprehensivFlowData.policyHolderEmail
      }
      if (comprehensivFlowData.policyHolderMobile) {
        this.flowData.policyHolderMobile = comprehensivFlowData.policyHolderMobile
      }
      if (comprehensivFlowData.insuredRelation) {
        this.flowData.insuredRelation = comprehensivFlowData.insuredRelation
      }
      if (comprehensivFlowData.policyHolder) {
        this.flowData.policyHolder = comprehensivFlowData.policyHolder
        this.policyHolder = comprehensivFlowData.policyHolder
      }
      if (comprehensivFlowData.insured) {
        this.flowData.insured = comprehensivFlowData.insured
        this.insured = comprehensivFlowData.insured
      }
      if (comprehensivFlowData.electricitySalesJobId) {
        this.flowData.electricitySalesJobId = comprehensivFlowData.electricitySalesJobId
      }
      this.flowData.socialInsuFlag = Boolean(comprehensivFlowData.socialInsuFlag)
    }
  },
  methods: {
    ...
    submitInfo () {
      if (this.userNameRules(this.policyHolder.name, '投保人') &&
        ...
        var comprehensivFlowData = {}
        comprehensivFlowData = $.extend(true, comprehensivFlowData, this.flowData)
        comprehensivFlowData.insuredRelation = this.getInsuredRelationType(comprehensivFlowData.insuredRelation)
        comprehensivFlowData.socialInsuFlag = Number(comprehensivFlowData.socialInsuFlag)

        if (this.hideUploadImage) {
          comprehensivFlowData.fromWhere = this.$route.query.fromWhere
          comprehensivFlowData.token = this.$route.query.token
          comprehensivFlowData.tokenUserId = this.$route.query.userId
        }
        ...
        var _this = this
        this.$http.post(this.host_bdd + '/lifeInsurance/v1/answern', comprehensivFlowData).then((rt) => {
          _this.$vux.loading.hide()
          if (rt.data.code !== 200) {
            _this.$vux.toast.text(rt.data.error, 'top')
            return
          ...
          _this.storage.set('anxinComprehensivFlowData', null)
          if (comprehensivFlowData.electricitySalesJobId && rt.data.inquirySms) {
            // 电销而来
    ...
    saveFlowData () {
      this.flowData.policyHolder = this.policyHolder
      this.flowData.insured = this.insured
      this.storage.set('anxinComprehensivFlowData', this.flowData)
    },
    inquirySmsConfirm () {
      var _this = this
      this.$vux.loading.show({
    ...
    goLianComplete () {
      this.storage.set('anxinComprehensivPayUrl', this.payUrl)
      this.$router.go(-1)
    }
  }
}
