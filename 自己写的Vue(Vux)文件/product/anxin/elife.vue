        ...
        <group :gutter='0'>
            ...
            :dialog-style="{'max-width': '90%', width: '90%' }">
                <div class="dialog-div">
                  <ol>
                    <li><p>1. 本人自愿投保贵公司《住院综合医疗保险条款（2017版）》，并声明以上陈述及各项细节均真实无讹，且没有隐瞒任何重大事实以影响贵公司评估风险或接受本投保申请。</p>
                    </li>
                    <li><p>2. 本人已认真阅读《住院综合医疗保险条款（2017版）》的所有条款，尤其是“不能获得赔偿的情形”并对贵公司就保险合同的内容说明和提示完全理解，没有异议，申请投保。</p>
                    </li>
                    <li><p>3. 本人知晓本投保申请将构成投保人与贵公司所签署的保险合同的依据，若未能披露与本保险相关之重大事实将可能导致贵公司不承担任何保险责任。保险合同生效日期以保险单所载生效日期为准，贵公司承担保险责任须以投保人缴付约定保险费并经贵公司同意承保为前提。</p>
                    </li>
                    <li><p>4. 本人同意贵公司为本保险的目的收集本人的个人资料，无论该资料是从本投保申请或其他地方所获取，并授权可由贵公司用于该保险的投保审核；提供与该保险有关之服务及与本人联络的用途。</p>
                    </li>
                  </ol>
                </div>
                <section class="submit-button">
            ...
            :dialog-style="{'max-width': '90%', width: '90%'}">
                <div class="dialog-div">
                  <div class="mdFormart_box_content_index">
                    <p>（1）适用人群：投保人18周岁及以上；被保险人6岁至45周岁； </p>
                    ...
                    <p>（7）被保险人非首次投保或续保时，累计本产品的已理赔金额应小于300万</p>
                    <p>（8）本保险自投保人投保支付成功后次日零时起生效。投保人投保后请尽快完成保费支付，未及时支付的订单将在投保当日24时自动失效，需重新点击购买；  </p>
                    <p>（9）本产品通过互联网在全国范围内销售；  </p>
                    <p>（10）本产品采用电子保单形式承保，法律效力等同于纸质保单。如果投保人需要纸质保单及发票，请拨打客服电话。若需寄送，邮费将由投保人自行承担；    </p>
                    <p>（11）温馨提示：安心互联网保险全国统一客服专线 95303；</p>
                    <p>（12）如实告知：投保人如投保本保险，应依法履行如实告知义务；<a class="show-pdf" @click='showPDF(1, "违反如实告知的后果！")'>违反如实告知的后果！</a></p>
                    <p>（13）投保本保险前，请投保人认真阅读产品页面展示内容以及《住院综合医疗保险条款（2017版）》。 </p>
                  </div>
                </div>
                <section class="submit-button">
            ...
            :dialog-style="{'max-width': '90%', width: '90%' }">
                <div class="dialog-div">
                  <a class="show-pdf" @click='showPDF(5, "《住院综合医疗保险（2017版）》")'>《住院综合医疗保险（2017版）》</a>
                </div>
                <section class="submit-button">
...
export default {
  ...
  created () {
    ...
    this.flowData.extraInfo = this.$route.query.extraInfo
    if (this.$route.query.fromWhere && this.$route.query.token && this.$route.query.userId) {
      this.hideUploadImage = true
    }
    var elifeFlowData = this.storage.get('anxinElifeFlowData')
    if (elifeFlowData) {
      if (elifeFlowData.produceId) {
        this.flowData.produceId = elifeFlowData.produceId
      }
      if (elifeFlowData.vendorId) {
        this.flowData.vendorId = elifeFlowData.vendorId
      }
      if (elifeFlowData.socialInsuFlag) {
        this.flowData.socialInsuFlag = elifeFlowData.socialInsuFlag
      }
      if (elifeFlowData.policyHolderEmail) {
        this.flowData.policyHolderEmail = elifeFlowData.policyHolderEmail
      }
      if (elifeFlowData.policyHolderMobile) {
        this.flowData.policyHolderMobile = elifeFlowData.policyHolderMobile
      }
      if (elifeFlowData.insuredRelation) {
        this.flowData.insuredRelation = elifeFlowData.insuredRelation
      }
      if (elifeFlowData.policyHolder) {
        this.flowData.policyHolder = elifeFlowData.policyHolder
        this.policyHolder = elifeFlowData.policyHolder
      }
      if (elifeFlowData.insured) {
        this.flowData.insured = elifeFlowData.insured
        this.insured = elifeFlowData.insured
      }
      if (elifeFlowData.electricitySalesJobId) {
        this.flowData.electricitySalesJobId = elifeFlowData.electricitySalesJobId
      }
      this.flowData.socialInsuFlag = Boolean(elifeFlowData.socialInsuFlag)
    }
  },
  methods: {
    ...
    insuredIdentityChange () {
      if (this.identityRules(this.insured.identity, '被保人').valid) {
        if (this.insured.identity.length === 18) {
          ...
          if (!this.judgeAge(this.insured.birth, 6, 80)) {
            this.$vux.toast.text('抱歉，暂时仅支持被保人6-80周岁的投保', 'top')
          }
        }
      }
    },
    ...
    submitInfo () {
      if (this.userNameRules(this.policyHolder.name, '投保人') &&
        ...
        var elifeFlowData = {}
        elifeFlowData = $.extend(true, elifeFlowData, this.flowData)
        elifeFlowData.insuredRelation = this.getInsuredRelationType(elifeFlowData.insuredRelation)
        elifeFlowData.socialInsuFlag = Number(elifeFlowData.socialInsuFlag)
        if (this.hideUploadImage) {
          elifeFlowData.fromWhere = this.$route.query.fromWhere
          elifeFlowData.token = this.$route.query.token
          elifeFlowData.tokenUserId = this.$route.query.userId
        }
        this.$vux.loading.show({
          text: '正在提交数据'
        })
        var _this = this
        this.$http.post(this.host_bdd + '/lifeInsurance/v1/answern', elifeFlowData).then((rt) => {
          _this.$vux.loading.hide()
          if (rt.data.code !== 200) {
            _this.$vux.toast.text(rt.data.error, 'top')
            return
          ...
          _this.storage.set('anxinElifeFlowData', null)
          if (elifeFlowData.electricitySalesJobId && rt.data.inquirySms) {
            // 电销而来
    ...
    saveFlowData () {
      this.flowData.policyHolder = this.policyHolder
      this.flowData.insured = this.insured
      this.storage.set('anxinElifeFlowData', this.flowData)
    },
    inquirySmsConfirm () {
      var _this = this
      this.$vux.loading.show({
    ...
    goLianComplete () {
      this.storage.set('anxinElifePayUrl', this.payUrl)
      this.$router.go(-1)
    }
  }
}
