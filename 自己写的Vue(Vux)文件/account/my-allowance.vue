...
<template>
    <div class="box">
        ...
        <incomeList
        :category=2
        :dateParam='dateTimeModel'
        :noData='noData'
        :marginTop='250' ></incomeList>
    </div>
</template>
...
export default {
  ...
  data () {
    return {
      ...
      dateTimeModel: new Date().getFullYear() + ''
    }
  },
  ...
  methods: {
                ...
                .then((rt) => {
                  ...
                  if (rt.data.catpitalAccount) {
                    ...
                    _this.headerModel.incomeAmount =
                        catpitalAccount.allowanceAmount > 0 ? catpitalAccount.allowanceAmount : '0.00'
                    var totalNum = '0.00'
                    if (catpitalAccount.totalAllowanceAmount >= 0) {
                      totalNum = catpitalAccount.totalAllowanceAmount
                      this.noData = false
                    } else {
                      this.noData = true
                    }
                    _this.headerModel.footerTitle = ('累计津贴 ' + totalNum + ' 万')
                  }
                })
    },
    ...
    getIncomeChartData () {
      var _this = this
      _this.$http.get(_this.host_bdd + '/captial/v1/allowance/chart')
                .then((rt) => {
                  if (rt.data.code !== 200) {
                    _this.$vux.toast.text(rt.error, 'top')
                    return
