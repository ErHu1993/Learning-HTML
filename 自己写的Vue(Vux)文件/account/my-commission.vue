...
import { GroupTitle,DatetimeView, Popup } from 'vux'
    ...
    export default {
        ...
        methods : {
                ...
                .then((rt) => {
                    if (rt.data.code != 200) {
                     ...
                     if (rt.data.catpitalAccount) {
                        ...
                        catpitalAccount.dayCommissionAmount > 0 ? catpitalAccount.dayCommissionAmount : "0.00";

                         var totalNum = "0.00"
                        if (catpitalAccount.totalCommissionAmount >= 0)  {
                            totalNum = catpitalAccount.totalCommissionAmount;
                            this.noData = false;
                        } else {
                            this.noData = true;
                        }
                        _this.headerModel.footerTitle = ("累计佣金 " + totalNum + ' 万');
                     }
                 });
            },
            ...
            getIncomeChartData () {
                var _this = this;
                _this.$http.get(_this.host_bdd + '/captial/v1/commission/chart')
                .then((rt) => {
                    if (rt.data.code != 200) {
                         _this.$vux.toast.text(rt.error, 'top');
                         return;
