    ...
    <div class="base" v-show='selectIndex == 1'>
        <div class="whiteColor">
            ...
            <div class="labels">
                ...
                <label class="right">直属上级真实姓名:     </label>
                <label class="right-content" v-text='agentDetail.parentContact'></label>
            </div>
            <div class="labels">
                <label class="left">添加时间:     </label>
                <label class="left-content" v-text='agentDetail.createTime'></label>
                <label class="right">代理人状态:     </label>
                <label class="right-content">{{ agentDetail.state | getAgnetStatus}}</label>
            </div>
      ...
      methods: {
          ...
          _this.$get(_this.host_bdd + '/agency/v1/inferior/detail/' + _this.id,
                function (rt) {
                  ...
                  if (_this.agentDetail.parent) {
                    ...
                    _this.agentDetail.parentContact = _this.agentDetail.parent.contact
                  } else {
                    _this.agentDetail.parentContent = ''
                    _this.agentDetail.parentContact = ''
                  }
        ...
        teamSearchTimeChange: function (results) {
          if (typeof (results[0] === 'string') && typeof (results[1] === 'string') && results[0].length && results[1].length) {
            ...
            this.teamEndSearchTime = parseInt(new Date(results[1]).getTime() / 1000 + 24 * 3600)
          } else {
            this.teamStartSearchTime = parseInt(this.options.shortcuts[2].value()[0].getTime() / 1000)
            this.teamEndSearchTime = parseInt(this.options.shortcuts[2].value()[1].getTime() / 1000)
          }
        ...
        analyseChartChange (index) {
          ...
          switch (index) {
            case 0 :// 佣金+保费+保单
              {
                ...
                let xAxisData = []// 月份
                let premiumSeriesData = []// 保费数据
                let policySeriesData = []// 保单个数据
                let commissionSeriesData = []// 佣金数据
                for (let i = 0; i < this.userStatisticsChartsData.lineChart.length; i++) {
                  let lineChart = this.userStatisticsChartsData.lineChart[i]
            ...
            case 1 :// 津贴
              {
                ...
                let xAxisData = []// 月份
                let allowanceSeriesData = []// 保费数据
                for (let i = 0; i < this.userAllowanceChartsData.lineChart.length; i++) {
                  let lineChart = this.userAllowanceChartsData.lineChart[i]
                  xAxisData.push(lineChart.datetime)
            ...
            case 2 : {
              ...
              let policySeriesData = []// 保单个数据
              let commissionSeriesData = []// 佣金数据
              for (let i = 0; i < this.teamStatisticsChartsData.lineChart.length; i++) {
                let lineChart = this.teamStatisticsChartsData.lineChart[i]
                xAxisData.push(lineChart.datetime)
                premiumSeriesData.push(lineChart.minor[0])
                commissionSeriesData.push(lineChart.minor[1])
                policySeriesData.push(lineChart.minor[2])
